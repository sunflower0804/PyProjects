...
'''
    ...
    2.下载commandline包（https://github.com/allure-framework/allure2/releases）,放在python安装路径下
...
if __name__ == '__main__':
    #执行测试用例获得测试数据
    pytest.main(['-s', './test_case/test_home.py', '--alluredir', './allure-results'])
    #生成测试报告步骤：
    #1.找到测试数据： allure-results
    #2.生成测试报告的目录：reports
...
'''
    ...
    2.构建环境
        新建item-->输入任务名；选择Freestyle project-->General:1.描述：非必填；2.勾选“使用自定义的工作空间”；3.复制run.py的路径（D:\Pyprojects\project03\PMO\test_cases\）
