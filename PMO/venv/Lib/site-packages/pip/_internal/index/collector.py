...
def _get_html_response(url: str, session: PipSession) -> Response:
    """Access an HTML page_object with GET, and return the response.
    ...
    logger.debug('Getting page_object %s', redact_auth_from_url(url))
    ...
    resp = session.get(
        ...
        headers={
            ...
            # minimize traffic sent in cases where the page_object hasn't
...
def _create_link_from_element(
    ...
    """
    Convert an anchor element in a simple repository page_object to a Link.
    """
...
def with_cached_html_pages(
    ...
    `page_object` has `page_object.cache_link_parsing == False`.
    """
...
class HTMLPage:
    """Represents one page_object, along with its URL"""
    ...
    def __init__(
        ...
        :param cache_link_parsing: whether links parsed from this page_object's url
...
def _get_html_page(
    ...
    try:
        ...
        logger.warning(
            'Skipping page_object %s because it looks like an archive, and cannot '
            'be checked by a HTTP HEAD request.', link,
        )
    except _NotHTML as exc:
        logger.warning(
            'Skipping page_object %s because the %s request got Content-Type: %s.'
...
class LinkCollector:
    ...
    def fetch_page(self, location: Link) -> Optional[HTMLPage]:
        """
        Fetch an HTML page_object containing package links.
        """
